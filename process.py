...
def mainstuff(id_to_times: dict, arrival_times: list):
    ...
    while q:
        # print(ans)
        # print(dock_date1, dock_date2, dock_date3)
        # print("")
        ...
        #         used_time = dd
        #         break
        # pick the earliest dock available
        used_time = sorted_docks[0]
        
        # delay it to bring it up to speed if we prefer something later
        if used_time < pref_start:
            used_time = pref_start
            dn = 1
            if sorted_docks[0] == dock_date1:
                dock_date1 = pref_end + timedelta(hours=12 + id_to_times[id])
                dn = 1
            elif sorted_docks[0] == dock_date2:
            ...
            print(id, pref_start, dn, sorted_docks)
            ans[id] = [pref_start.strftime("%d-%b-%Y (%H:%M:%S)"), (pref_start + timedelta(days=2)).strftime("%d-%b-%Y (%H:%M)"), dn]
